...
def load_pretrained_uncond(filename):
    ####################################################
    ...
    lr_model.load_state_dict(torch.load('saved_model/' + filename)['model'])
    return lr_model, hidden_size

def load_pretrained_congen(filename):
    ####################################################
    # default parameters, do not change
    ...
    lr_model.load_state_dict(torch.load('saved_model/'+ filename, map_location=device)['model'])
